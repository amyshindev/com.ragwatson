...
### 1.1 한 줄 정의
중세 영어(Middle English)·르네상스 영문학 텍스트를 입력하면 **① 현대 영어 직역 ② 한국어 의역 ③ 사회·문화적 맥락 주석**을 동시에 제공하고, 르네상스 소네트의 rhetorical device를 자동 분석·시각화하는 AI 기반 층위별(Layered) 번역·해설 서비스.
...
- 르네상스 소네트에 빈출하는 rhetorical device (figurative language, literary devices, stylistic elements 등) 자동 분석
- 원관념-보조관념 매핑을 시각적으로 설명

### 3.3 대상 텍스트 범위 (단계별)
- **MVP**: 셰익스피어 소네트 일부
- **Phase 2**: 초서 Canterbury Tales 발췌 (중세 영어 격변화 분석 포함)
- **Phase 3+**: 형이상학파 시인(John Donne 등) 확장
...
### 7.3 데이터
- 원문: 퍼블릭 도메인 텍스트
- Folger Editions 주석: 작품별 grounding 소스로 로컬 보관 (출처 메타데이터 포함)
- 번역/주석 결과: 작품-구절 단위로 DB에 저장, 누적 주석 DB화
- 참고자료 메타데이터 테이블: Folger/일본/한국 선행 연구 출처 기록
...
### 8.1 범위
- 텍스트: 셰익스피어 소네트 5편으로 시작
  - Sonnet 18 (Petrarchan figurative language 대표 사례)
  - Sonnet 130 (anti-Petrarchan — rhetorical device 전복 사례, Sonnet 18과 대비)
  - Sonnet 116 (나침반 이미지의 확장 은유)
  - Sonnet 73 (사계절·황혼·불씨로 이어지는 다층 이미지 — 층위별 출력 데모에 적합)
  - Sonnet 12 (시계·꽃·낫의 시간/소멸 이미지)
- 각 소네트의 **Folger Shakespeare Editions 주석을 1차 grounding 자료**로 활용
- 기능: 3단 레이어 출력 + rhetorical analysis 1종(Petrarchan figurative language)부터
- 중세 영어(초서) 격변화 분석은 Phase 2로 이동 (기술 난이도, Folger 자산 연계성 모두 셰익스피어 쪽이 우선)

### 8.2 완료 기준 (Definition of Done)
- [ ] 소네트 5편에 대해 3단 레이어 출력이 일관된 형식으로 생성됨
- [ ] Rhetorical Analysis 결과가 tenor·vehicle 다이어그램으로 표현됨
- [ ] 주석에 참고 출처(Folger/일본 자료 등)가 구조화되어 기록됨
...
| 기간 | 내용 |
|---|---|
| 1~2주 | 원문(소네트 5편) 정리, Folger 주석·일본 자료 조사·정리 |
| 3~5주 | 도메인 모델 설계(헥사고날), DB 스키마 |
| 6~8주 | AI 프롬프트 설계·검증 (Folger grounding 포함 3단 레이어 + rhetorical analysis) |
| 9~11주 | 프론트엔드 MVP, 주석 DB 연동 |
| 12주~ | 영문학 전공자 피드백 → 텍스트 범위 확장 |
...
## 10. 향후 확장 (Phase 2+)
- 초서 Canterbury Tales 등 중세 영어 텍스트 + 격변화 분석
- 형이상학파 시(John Donne 등) 확장
- (장기) PPCME2/MED 등 학술 코퍼스 연동으로 정확도 고도화
