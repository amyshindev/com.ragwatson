...
### 1.1 한 줄 정의
중세 영어(Middle English)·르네상스 영문학 텍스트를 입력하면 **① 현대 영어 직역 ② 한국어 의역 ③ 사회·문화적 맥락 주석**을 동시에 제공하고, 르네상스 소네트의 수사학적 장치(conceit 등)를 자동 탐지·시각화하는 AI 기반 층위별(Layered) 번역·해설 서비스.
...
| 수사학 분석 | 설명 자료만 존재, 자동 탐지 도구 없음 | Conceit 등 수사학적 장치 자동 탐지·시각화 |
...
### 3.2 수사학적 장치 분석 (Conceit Detection)
- 르네상스 소네트에 빈출하는 정형화된 비유(Petrarchan conceit 등) 자동 탐지
...
**핵심 공백**: "중세/르네상스 영문학 + 한국어 레이어 + 문화적 주석 + conceit 자동탐지" 통합 서비스 부재.
...
- **역할**: "사회·문화적 주석" 레이어의 영어권 학술 기준점. Conceit·암시·언어유희 등 수사적 장치가 본문 어디에 있는지 식별하는 출발점으로 활용
...
- 헥사고날 아키텍처: 도메인(원문/번역/주석/conceit) ↔ AI 어댑터(Claude API) ↔ 영속성 계층 분리

### 7.2 AI 레이어
- Claude API: 층위별 번역(직역/의역/주석) 생성 및 conceit 탐지·설명
- 작품/장르별 프롬프트 템플릿(소네트 vs 희곡 vs 중세 시) 분리 관리
- **Folger Editions의 해당 작품 주석을 프롬프트 컨텍스트로 제공(grounding)하여 주석·conceit 탐지 정확도 확보 — MVP부터 적용**
...
- 텍스트: 셰익스피어 소네트 5편으로 시작
  - Sonnet 18 (대표적 Petrarchan conceit)
  - Sonnet 130 (conceit의 패러디/전복 — 18과 대비 효과)
...
- 기능: 3단 레이어 출력 + conceit 탐지 1종(Petrarchan)부터
...
- [ ] Conceit 탐지 결과가 원관념-보조관념 다이어그램으로 표현됨
...
| 6~8주 | AI 프롬프트 설계·검증 (Folger grounding 포함 3단 레이어 + conceit 탐지) |
